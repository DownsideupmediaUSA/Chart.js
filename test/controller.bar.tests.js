...
describe('Bar controller tests', function() {
	...
	it('should update elements when the scales are stacked and the y axis is logarithmic', function() {
		...
		[
			{b: 484, w: 93, x: 94, y: 379},
			{b: 484, w: 93, x: 208, y: 122},
			{b: 484, w: 93, x: 322, y: 379},
			{b: 484, w: 93, x: 436, y: 122}
		].forEach(function(values, i) {
			expect(meta0.data[i]._model.base).toBeCloseToPixel(values.b);
			expect(meta0.data[i]._model.width).toBeCloseToPixel(values.w);
			expect(meta0.data[i]._model.x).toBeCloseToPixel(values.x);
		...
		[
			{b: 379, w: 93, x: 94, y: 109},
			{b: 122, w: 93, x: 208, y: 109},
			{b: 379, w: 93, x: 322, y: 379},
			{b: 122, w: 93, x: 436, y: 25}
		].forEach(function(values, i) {
			expect(meta1.data[i]._model.base).toBeCloseToPixel(values.b);
			expect(meta1.data[i]._model.width).toBeCloseToPixel(values.w);
			expect(meta1.data[i]._model.x).toBeCloseToPixel(values.x);
